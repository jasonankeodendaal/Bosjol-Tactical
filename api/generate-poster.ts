  ...
  try {
    ...
    if (!apiKey) {
      ...
      return res.status(200).json({
        ...
        notice: 'GEMINI_API_KEY environment variable is not detected on Vercel. In Vercel Project Settings > Environment Variables, add GEMINI_API_KEY (and redeploy). Displaying high-definition tactical base plate in the meantime.'
      });
    }
...
Lighting: Volumetric tactical smoke, neon accents, dramatic rim lighting, extreme depth of field, heavy grunge textures. Clean cinematic background artwork without flat text overlay. Aspect ratio 3:4 portrait. ${prompt ? `Theme: ${prompt}` : ''}`;
    ...
    for (const modelName of modelsToTry) {
      try {
        ...
        lastError = err;
        console.warn(`[Vercel Serverless] Attempt with ${modelName} failed:`, err?.message || err);
      }
    }

    if (!imageUrl) {
      const fallbackImage = FALLBACK_TACTICAL_IMAGES[subjectType] || FALLBACK_TACTICAL_IMAGES.tactical_operator;
      const errorMsg = lastError?.message || 'Quota limit reached or service busy. Using high-definition tactical base plate.';
      return res.status(200).json({
        success: true,
        imageUrl: fallbackImage,
    ...
    console.error('[Vercel Serverless] Error generating poster:', err);
