...
// --- Shared Helper Components for Guides ---
...
type GuideType = 'hybrid' | 'ultimate';

export const ApiSetupTab: React.FC<ApiSetupTabProps> = ({ creatorDetails }) => {
    const [activeGuide, setActiveGuide] = useState<GuideType>('hybrid'); // Default to hybrid

    const renderHybridGuide = () => (
  ...
  frontend:
    ...
    return (
        <div className="space-y-8 max-w-4xl mx-auto">
            <DashboardCard title="API Server Setup Guides" icon={<CodeBracketIcon className="w-6 h-6" />}>
                <div className="p-4 sm:p-6">
                    <div className="mb-6 flex flex-wrap gap-3">
                        <Button
                            variant={activeGuide === 'hybrid' ? 'primary' : 'secondary'}
                            onClick={() => setActiveGuide('hybrid')}
                            size="sm"
                        >
                            Hybrid Media Server (Cloudflare)
                        </Button>
                        <Button
                            variant={activeGuide === 'ultimate' ? 'primary' : 'secondary'}
                            onClick={() => setActiveGuide('ultimate')}
                            size="sm"
                        >
                            Ultimate Self-Hosted Server (Oracle)
                        </Button>
                    </div>
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        >
                            {activeGuide === 'hybrid' && renderHybridGuide()}
                            {activeGuide === 'ultimate' && renderUltimateGuide()}
                        </motion.div>
                    </AnimatePresence>
                </div>
            </DashboardCard>
