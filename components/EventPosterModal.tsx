...
function formatGeminiErrorMessage(raw: any, cooldownSec = 45): string {
  ...
  if (str.includes('429') || str.includes('quota') || str.includes('RESOURCE_EXHAUSTED')) {
    return `High-definition 3D tactical base plate active. Custom AI artwork is on a brief cooldown (${cooldownSec}s). You can use this HD plate, switch baseplate styles below, or upload your own field photo.`;
  }

  if (str.includes('API_KEY_INVALID') || str.includes('403') || str.includes('PERMISSION_DENIED')) {
    return 'Invalid Gemini API Key: Please verify GEMINI_API_KEY in Environment Variables.';
  }
...
export const EventPosterModal: React.FC<EventPosterModalProps> = ({
  ...
  const handleGenerateAiPoster = async (overrideSubject?: any) => {
    ...
    if (cooldownSec > 0) {
      ...
      setAiNotice(`Google Gemini Rate Limit (429): Per-minute generation limit reached. High-definition tactical base plate active. Retry available in ${cooldownSec}s or upload your own field photo.`);
      return;
    }
    ...
    try {
      ...
      // If server provided a cooldown timer duration (e.g. 45s), activate local countdown
      if (data && typeof data.cooldownSec === 'number' && data.cooldownSec > 0) {
      ...
      if (data && data.imageUrl) {
        ...
        if (data.apiKeyConfigured === false) {
          ...
          setAiNotice(formatGeminiErrorMessage(data.notice, data.cooldownSec || cooldownSec || 45));
      ...
      // Case 2: Fallback smoothly to scenario tactical base plate
      setPosterBgUrl(clientFallback);
      if (res?.status === 404 || networkError) {
        setAiNotice(
          'Vercel Deployment Notice: The serverless endpoint /api/generate-poster was not found on this deployment. Please verify GEMINI_API_KEY in Vercel Environment Variables.'
        );
      } else {
        setAiNotice(formatGeminiErrorMessage(data?.notice || 'High-definition tactical base plate active. Set GEMINI_API_KEY in Environment Variables.'));
      }
  ...
  // Load initial artwork on mount without triggering rate-limited API calls automatically
  useEffect(() => {
    if (!autoGeneratedRef.current) {
      autoGeneratedRef.current = true;
      try {
        const saved = localStorage.getItem(`bosjol_poster_bg_${event.id}`);
        if (saved) {
          setPosterBgUrl(saved);
          return;
        }
      } catch {
      ...
      if (!event.imageUrl || event.imageUrl === companyDetails.logoUrl) {
        const clientFallback = CLIENT_TACTICAL_BASEPLATES[subjectPreset] || CLIENT_TACTICAL_BASEPLATES.tactical_operator;
        setPosterBgUrl(clientFallback);
      }
    }
  }, []);
          ...
          <div className="lg:col-span-5 flex flex-col justify-between space-y-4">
            <div className="space-y-4 overflow-y-auto max-h-[72vh] pr-1">
              ...
              <div className="bg-zinc-950/90 p-3.5 rounded-xl border border-zinc-800 space-y-3">
                ...
                {/* 1-Click Base Plate Preset Selector (Instant 0ms Swap, No Rate Limit Errors) */}
                <div className="space-y-1.5 pt-1">
                  <span className="text-[10px] font-mono text-zinc-400 uppercase tracking-wider block">
                    Instant 3D Base Plate Styles (0ms Lag, No Rate Limit):
                  </span>
                  <div className="grid grid-cols-5 gap-1.5">
                    {[
                      { id: 'tactical_operator', label: 'Operator', icon: '🛡️' },
                      { id: 'tactical_ape', label: 'Ape', icon: '🦍' },
                      { id: 'ghillie_sniper', label: 'Sniper', icon: '🎯' },
                      { id: 'heavy_juggernaut', label: 'Juggernaut', icon: '💣' },
                      { id: 'dual_faceoff', label: 'Faceoff', icon: '⚔️' }
                    ].map(preset => (
                      <button
                        key={preset.id}
                        type="button"
                        onClick={() => {
                          setSubjectPreset(preset.id as any);
                          const newBg = CLIENT_TACTICAL_BASEPLATES[preset.id] || CLIENT_TACTICAL_BASEPLATES.tactical_operator;
                          setPosterBgUrl(newBg);
                          setAiNotice(null);
                        }}
                        className={`py-1.5 px-1 rounded-lg border text-[10px] font-bold transition flex flex-col items-center gap-0.5 ${
                ...
                <button
                  ...
                  disabled={isGeneratingAi || cooldownSec > 0}
                  className="w-full py-2.5 px-3 rounded-lg bg-gradient-to-r from-red-600 to-amber-600 hover:from-red-500 hover:to-amber-500 text-white font-black text-xs uppercase tracking-wider transition flex items-center justify-center gap-2 shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isGeneratingAi ? (
                    <>
                      <Loader2 className="w-4 h-4 animate-spin" />
                      <span>Synthesizing Custom AI Artwork...</span>
                    </>
                  ) : cooldownSec > 0 ? (
                    <>
                      <Clock className="w-4 h-4 text-amber-300 animate-pulse" />
                      <span>⏳ AI Cooldown Active ({cooldownSec}s)</span>
                    </>
                  ) : (
                    <>
                      <Sparkles className="w-4 h-4 text-amber-300" />
                      <span>⚡ Generate Custom AI Background</span>
                    </>
                  )}
                </button>

                {aiNotice && (
                  <div className="p-3 rounded-xl bg-amber-950/70 border border-amber-500/40 text-amber-200 text-xs flex items-start gap-2.5 shadow-md relative">
                    ...
                    <div className="space-y-1.5 flex-1 pr-4">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-amber-900/60 border border-amber-600/40 text-amber-300 font-bold">
                          {cooldownSec > 0 ? `⏳ ${cooldownSec}s COOLDOWN` : 'HD READY'}
                        </span>
                      </div>
                      ...
                      <div className="pt-1.5 flex flex-wrap items-center gap-2">
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded bg-amber-900/50 hover:bg-amber-900 border border-amber-500/40 text-[11px] font-bold text-amber-200 transition"
                        >
                          <Zap className="w-3.5 h-3.5 text-amber-400" />
                          <span>Use High-Res Base Plate</span>
                        </button>

                        <label className="cursor-pointer inline-flex items-center gap-1.5 px-2.5 py-1 rounded bg-zinc-900 hover:bg-zinc-800 border border-zinc-700 text-[11px] font-medium text-zinc-300 transition hover:border-zinc-500">
                    ...
                    <button
                      ...
                      className="absolute top-2 right-2 text-zinc-400 hover:text-white text-xs p-1 rounded-full hover:bg-amber-900/50 transition"
