...
// ==========================================
const WinnerCelebrationModal: React.FC<{
    win: {
        raffleName: string;
        prize?: Prize;
    ...
    onClose: () => void;
    onOpenArena: (r: Raffle) => void;
}> = ({ win, player, onClose, onOpenArena }) => {
    useEffect(() => {
        try {
    ...
    return (
        <div className="fixed inset-0 z-[250] bg-black/90 backdrop-blur-2xl flex items-center justify-center p-4">
            <motion.div
                ...
                exit={{ scale: 0.8, opacity: 0 }}
                className="w-full max-w-lg bg-gradient-to-b from-amber-950/90 via-zinc-900 to-black p-6 sm:p-8 rounded-3xl border-2 border-amber-400 shadow-[0_0_80px_rgba(245,158,11,0.5)] text-center relative overflow-hidden"
            >
                {/* 3D Top Glow */}
                <div className="absolute -top-32 left-1/2 -translate-x-1/2 w-80 h-80 bg-amber-500/30 rounded-full blur-3xl pointer-events-none" />

                <div className="relative z-10 space-y-4">
                    <div className="w-20 h-20 mx-auto rounded-full bg-gradient-to-tr from-amber-500 to-yellow-300 p-0.5 shadow-[0_0_30px_rgba(245,158,11,0.8)] flex items-center justify-center">
                    ...
                    <div>
                        <span className="font-mono text-xs font-black uppercase tracking-widest text-amber-400 bg-amber-950/80 px-3 py-1 rounded-full border border-amber-500/40">
                            OFFICIAL RAFFLE WINNER!
                        </span>
                    ...
                    <div className="p-4 rounded-2xl bg-black/80 border border-amber-500/50 shadow-inner space-y-2">
                        <div className="text-xs font-mono text-zinc-400">RAFFLE EVENT: <strong className="text-white">{win.raffleName}</strong></div>
                        <div className="text-lg font-black text-amber-400 flex items-center justify-center gap-2">
                            <span>🎁</span>
                            <span>{win.prize?.name || 'Grand Tactical Prize'}</span>
                        </div>
                        {win.ticket && (
                            <div className="inline-block bg-amber-950/60 border border-amber-500/40 px-3 py-1 rounded-xl text-xs font-mono font-bold text-red-400">
                                Winning Ticket: {win.ticket.code}
                            </div>
                        )}
                    </div>

                    <p className="text-xs text-zinc-400 leading-relaxed">
                        Please present your winning ticket code or player callsign to the event administrator or field organizer to collect your prize.
                    </p>
                    ...
                    <div className="pt-2 flex flex-col sm:flex-row gap-2">
                        <Button
                            onClick={() => {
                                onClose();
                                onOpenArena(win.raffle);
                            }}
                            className="flex-1 bg-gradient-to-r from-amber-600 to-red-600 text-white font-black py-3 rounded-xl shadow-lg text-xs uppercase"
                        >
                            Open Live Arena
                        </Button>
                        <Button
                            onClick={onClose}
                            variant="secondary"
                            className="flex-1 py-3 rounded-xl text-xs uppercase text-zinc-300"
                        >
                            Acknowledge & Close
                        </Button>
                    </div>
        ...
        <div className="space-y-6">
            ...
            {celebrationWin && (
                <WinnerCelebrationModal
                    win={celebrationWin}
                    player={player}
                    onClose={() => setCelebrationWin(null)}
