...
export const SystemScanner: React.FC = () => {
    ...
    const ALL_CHECKS = useCallback((filterPlayerId?: string): Check[] => {
        ...
        if (!filterPlayerId) {
             checks = checks.concat([
                ...
                { category: 'Data Integrity', name: 'Stale Event Status', description: 'Finds past events still marked as Upcoming.', checkFn: async () => { const now = new Date(); const stale = events.filter(e => (new Date(e.date) < now) && e.status === 'Upcoming'); return stale.length > 0 ? { status: 'warn', detail: `${stale.length} past event(s) still marked 'Upcoming'.`, fixable: true } : { status: 'pass', detail: 'No stale event statuses found.' }; }, fixFn: async (d) => { const now = new Date(); const stale = d.events.filter(e => (new Date(e.date) < now) && e.status === 'Upcoming'); for (const event of stale) { await d.updateDoc('events', { ...event, status: 'Completed' }); const associatedSignups = d.signups.filter(s => s.eventId === event.id); for (const signup of associatedSignups) { await d.deleteDoc('signups', signup.id); } } }},
    ...
    const handleFixAll = async () => {
        // FIX: In some TypeScript configurations, `flat()` can have poor type inference.
        // We explicitly cast the result to ensure type safety.
        const fixableErrors = (Object.values(results).flat() as CheckResult[]).filter(r => r.fixable && (r.status === 'fail' || r.status === 'warn'));
